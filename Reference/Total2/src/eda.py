...
def perform_numeric_eda(
    df: pd.DataFrame, numeric_columns: list[str], income_column: str = "income"
) -> pd.DataFrame:
    """수치형 컬럼별 기술통계(사분위수/IQR, 왜도, 첨도, 이상치 후보 등)를 계산하고 해석 힌트를 출력한다."""
    summary_rows = []
    income_group_rows = []
    for col in numeric_columns:
        ...
        outlier_count = int(((valid_values < lower_bound) | (valid_values > upper_bound)).sum())
        ...
        if income_column in df.columns:
            high_income_values = df.loc[df[income_column] == ">50K", col].dropna()
            low_income_values = df.loc[df[income_column] == "<=50K", col].dropna()
            income_group_rows.append(
                {
                    "column": col,
                    "mean_>50K": high_income_values.mean(),
                    "median_>50K": high_income_values.median(),
                    "mean_<=50K": low_income_values.mean(),
                    "median_<=50K": low_income_values.median(),
                }
            )
    ...
    if income_group_rows:
        income_group_df = pd.DataFrame(income_group_rows).set_index("column").round(2)
        print("\n[소득 그룹별 평균/중앙값 비교]")
    ...
    for col in numeric_columns:
        ...
        print(
            ...
            f"왜도={row['skewness']:.2f}({skew_desc}), 0값 비율={row['zero_ratio(%)']:.1f}%, "
            f"이상치 후보={int(row['outlier_count'])}건"
        )
...
def perform_categorical_eda(
    ...
    """범주형 컬럼별 고유값 개수, 최빈값, 상위 범주 비율, 결측치, 희소 범주, 범주별 >50K 인원수/비율을 출력한다."""
    print("\n[범주형 컬럼 EDA]")
    for col in categorical_columns:
        ...
        show_income_breakdown = income_column in df.columns and col != income_column

        print(f"\n- {col}")
        print(f"  고유값 개수: {unique_count} (상위 {min(top_n, unique_count)}개만 출력)")
        print(f"  최빈값: {mode_value}")
        print(f"  결측치 개수: {missing_count} ({missing_ratio:.2f}%)")
        for category, count in value_counts.items():
            line = f"    {category}: {count}건 ({value_ratios[category]}%)"
            if show_income_breakdown:
                category_mask = df[col] == category
                high_income_count = int((category_mask & (df[income_column] == ">50K")).sum())
                high_income_ratio = high_income_count / count * 100 if count else 0.0
                line += f" | >50K: {high_income_count}건 ({high_income_ratio:.2f}%)"
            print(line)

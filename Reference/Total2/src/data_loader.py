...
def compare_pandas_polars(
    ...
    pandas_load_seconds: float | None = None,
    polars_load_seconds: float | None = None,
) -> None:
    """Pandas와 Polars로 각각 읽은 결과의 주요 지표가 일치하는지 비교 출력한다."""
    print("\n[Pandas vs Polars 비교]")

    if pandas_load_seconds is not None and polars_load_seconds is not None:
        print("\n[로딩 실행 시간 비교]")
        print(f"Pandas: {pandas_load_seconds:.4f}초 / Polars: {polars_load_seconds:.4f}초")

    print("\n[메모리 사용량 비교]")
    pandas_memory_mb = pandas_df.memory_usage(deep=True).sum() / (1024 ** 2)
    polars_memory_mb = polars_df.estimated_size() / (1024 ** 2)
    print(f"Pandas: {pandas_memory_mb:.2f}MB / Polars: {polars_memory_mb:.2f}MB")

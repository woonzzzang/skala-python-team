...
def generate_markdown_report(context: dict[str, Any], report_path: Path) -> Path:
    ...
    additional_t_test_lines = "\n".join(
        f"  - {test['group_column']} ({test['group_a_label']} vs {test['group_b_label']}) x "
        f"{test['value_column']}: {test['test_type']}, 통계량={test['statistic']:.4f}, "
        f"p-value={test['p_value']:.4f}, Cohen's d={test['cohens_d']:.3f}, "
        f"{'유의함' if test['is_significant'] else '유의하지 않음'}"
        for test in context.get("additional_t_tests", [])
    )
    ...
    model_comparison_lines = "\n".join(
        ...
        for name, metrics in model_info.get("candidate_comparison", {}).items()
    )
...
{additional_t_test_lines if additional_t_test_lines else '  - 수행하지 않음'}
...
- 비교한 후보 모델:
{model_comparison_lines if model_comparison_lines else '  - 비교 없이 단일 모델만 학습'}
- 최종 선택 모델: {model_info.get('final_model_name', 'LogisticRegression')}
...
{output_files['additional_distribution_png']}, {output_files['categorical_eda_png']}, \
...
{output_files['plotly_occupation_html']}, {output_files['plotly_facet_html']}, \
{output_files['plotly_treemap_html']}, {output_files['plotly_parallel_html']}
